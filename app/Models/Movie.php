...
//        var_dump($result[0]["title"]);exit;
    ...
    {
        $result = DB::select("select original_title,countries,year,aka,url_douban from movies_details where id = ?",[$id]);
        return $result[0];
    }
    ...
    {
        $result = DB::select("select type_name from movies_genres , movies_genres_details where movies_id = ? and movies_genres.genres_id = movies_genres_details.genres_id",[$id]);
        return $result;
    }

    public static function movies_directors_select( $id )
    {
        $result = DB::select("select name from movies_director , movies_directors where movies_id = ? and movies_director.id = movies_directors.directors_id",[$id]);
        return $result;
    }

    public static function movies_casts_select( $id )
    {
        $result = DB::select("select name from actors , movies_actors where movies_id = ? and actors.id = movies_actors.actors_id",[$id]);
        return $result;
    }
